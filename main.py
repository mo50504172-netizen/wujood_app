...
# --- 2. الماستر بلان التفاعلية (تم تعديل الـ Hover هنا) ---
...
if not df_all.empty:
    try:
        ...
        # تجميع الوحدات اللي ليها نفس الإحداثيات عشان تظهر مع بعض
        df_grouped = df.groupby(['X', 'Y']).apply(lambda x: x.to_dict('records')).reset_index(name='units')
        
        # تجهيز النص اللي هيظهر لما تقف بالماوس (بدون Trace 1 وبدون إحداثيات)
        hover_texts = []
        ...
        if os.path.exists("Master Plan.jpeg"):
            ...
            # إضافة النقط الحمراء مع تخصيص الـ Hover
            fig.add_scatter(
                x=df_grouped['X'], 
                y=df_grouped['Y'], 
                mode='markers', 
                ...
                hoverinfo="text", # دي اللي بتشيل Trace 1 والإحداثيات وبتعرض نصنا بس
                name="" # بتخلي اسم الطبقة فاضي عشان ميبوخش الشكل
            )
            ...
            fig.update_layout(
                ...
                showlegend=False # إخفاء قائمة الرموز الجانبية
            )
            fig.update_xaxes(visible=False); fig.update_yaxes(visible=False)
            st.plotly_chart(fig, use_container_width=True)
    except Exception as e: st.error(f"Masterplan Error: {e}")

# --- 3. صانع العروض (Offer Builder) ---
with st.sidebar:
