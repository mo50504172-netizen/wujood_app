...
def load_data():
    ...
    data.columns = data.columns.str.strip() # تنظيف أسماء الأعمدة من أي مسافات
    return data

# 3. دالة بناء الـ PDF المعدلة (بدون خطأ الـ encode)
def create_pdf(unit_row, consultant_name):
    ...
    if os.path.exists("Master Plan.jpeg"):
        ...
        # تحويل الإحداثيات لمقاس الـ PDF
    ...
    return pdf.output(dest='S') # إرجاع البيانات كـ bytes مباشرة
...
try:
    ...
    with tab1:
        ...
        # تجميع الوحدات في نفس النقطة لضمان ظهور الجميع
        ...
        # عرض الماستر بلان بنظام imshow لضمان دقة الإحداثيات
            ...
            with st.spinner("Preparing PDF..."):
                try:
                    ...
                    st.download_button(
                        ...
                        data=pdf_bytes, 
                        file_name=f"Wujood_Offer_{unit_to_offer}.pdf",
